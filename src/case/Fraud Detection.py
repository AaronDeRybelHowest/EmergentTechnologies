...
'''
Drop the the column 'Time' and store your result. 
...
'''
This dataset was mostly anonimized, only the collumns Time and amount are original. 
...
Random number generators are functions that computers use to generate fake random numbers.
You can choose the random number(s) generated by choosing "the seed" of the random number generator.

In the code below the function shuffle is used. Everytime it's called it returns a randomly
shuffled dataset. Do this for df_normal and df_fraud.

Also to keep the same random shuffling of the dataset "the seed" should be chosen, so fill in random_state
with an integer of your choice. This way your machine learning model will produce the same results. 

Note: The shuffle function will return a pandas dataframe if a pandas dataframe is given.
      You can try different random_state values to see if your model is consistent. 
'''
...
setting p=1,2,3 gives different metrics if metric='minkowski' and weights='distance'. This is the default value for the metric argument
...
Questions:
1) Data reading: After dropping the collumn 'Time', what collumns do we have left?
2) Data preprocessing: We've dropped the collumn 'Time'. What information do you lose when doing this?
If we keep the 'Time' and shuffle the dataset what information do we lose instead ? 
...
5) copy the printout of the dictionary with confusion matrix values made in the last code-block, for your best and second best results.
'''
